...
public class PathFollower extends CommandBase {
  /**
   * An enum for the column for variables in the CSV generated by bob trajectory
   */
  public enum CSV_VALS{
  ...
  public void execute() {
    ...
    ,Double.parseDouble(right.get(currentLine)[GenPathSetup.vel()]) * (120 * 11.111 / Math.PI), 
    1.75*Double.parseDouble(center.get(currentLine)[GenPathSetup.vel()])  * heading_difference
    ,Double.parseDouble(left.get(currentLine)[GenPathSetup.acc()]),Double.parseDouble(right.get(currentLine)[GenPathSetup.acc()]));
